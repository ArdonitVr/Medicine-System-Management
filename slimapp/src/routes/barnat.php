...
// Add cutomer
$app->post('/api/barna/shto', function (Request $request, Response $response) {
	...
	try{
		...
		$stmt->execute();
		echo'{"notice":{"text":"barna u shtua"}';
	} catch(PDOException $e){
		echo '{"error":{"text": '.$e->getMessage().'}';
	}
});
...
// Update barna
$app->put('/api/barna/modifiko/{ID_SMBR}', function (Request $request, Response $response) {
	//To get the barna ID_SMBR create variable
	$ID_SMBR = $request->getAttribute('ID_SMBR');
	$Emri_Barnes_SMBR = $request->getParam('Emri_Barnes_SMBR');
	$ProdhuesiBarnes_SMBR = $request->getParam('ProdhuesiBarnes_SMBR');
	...
	try{
		...
		$stmt->bindParam(':LlojiBarnes_SMBR',$LlojiBarnes_SMBR);
		$stmt->execute();
		echo'{"notice":{"text":"barna u modifikua"}';
	} catch(PDOException $e){
		echo '{"error":{"text": '.$e->getMessage().'}';
	}
});
...
// Delete barna
$app->delete('/api/barna/fshiej/{ID_SMBR}', function (Request $request, Response $response) {
	//To get the barna ID_SMBR create variable
	$ID_SMBR = $request->getAttribute('ID_SMBR');
	...
	try{
		...
		$stmt = $db->prepare($sql);
		$stmt->execute();
		echo'{"notice":{"text":"barna u fshiej"}';
